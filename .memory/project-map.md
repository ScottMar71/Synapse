...
- `apps/web/app/admin/courses/[courseId]/`: course editor — metadata (`PATCH /courses/:id`), category links (`PUT .../categories`), publish + archive; **lesson external links** authoring in `lesson-links-editor.tsx` (`GET|POST|PATCH|DELETE .../lessons/:lessonId/links`); **lesson file attachments** authoring in `lesson-files-editor.tsx` (list, `POST .../files/upload-init` + presigned PUT with progress UI, `PATCH .../files` reorder, `PATCH|DELETE .../files/:fileId`, staff download); **lesson glossary** authoring in `lesson-glossary-editor.tsx` (`GET|POST|PATCH|DELETE .../lessons/:lessonId/glossary`); staff-only gate via `probeInstructorRoute` before load. Layout preview: `player-wireframe/`.
